...
def rlaifv_collate_fn(batch):
    ...
    for key in keys_to_pad:
        ...
        collated[key] = pad_sequence(
            tensors, batch_first=True, padding_value=pad_values[key]
        )

    # pixel_values 形状一致 (1,3,448,448)，直接 stack
    collated["pixel_values"] = torch.stack(
        [item["pixel_values"].squeeze(0) for item in batch], dim=0
    )
    collated["thinking_mode"] = [item["thinking_mode"] for item in batch]
    return collated
...
class GRPOTrainer:
    ...
    def compute_grpo_loss(
        ...
        for b in range(batch_size):
            ...
            advantage_expanded = advantage.unsqueeze(1).expand_as(ratio)
...
class Stage4RLTrainer:
    ...
    def __init__(
        ...
        self.rlaifv_loader = DataLoader(
            ...
            num_workers=0, drop_last=True,collate_fn=rlaifv_collate_fn

        )

        print(f"[Stage4] rlpr 样本数={len(rlpr_dataset)}, rlaifv 样本数={len(rlaifv_dataset)}")
        ...
        if self.is_main_process:
            print(f"[Stage4] 训练精度: {self.dtype}, GradScaler: {'启用' if self.scaler else '禁用（bfloat16 不需要）'}")
    ...
    def _backward_and_step(self, loss: torch.Tensor, optimizer: AdamW, step_scheduler: bool = False):
        """
        统一的 backward + clip + step，处理 scaler=None 的情况。
        """
    ...
    def train(self):
        ...
        while step < self.total_rl_steps:
            ...
            if use_grpo:
                ...
                if batch is None:
                    print("[Warning] rlpr 数据集为空，跳过本步 GRPO 更新")
                    step += 1
                    continue
                ...
                self._backward_and_step(grpo_loss, self.grpo.optimizer, step_scheduler=False)
                ...
                if batch is None:
                    print("[Warning] rlaifv 数据集为空，跳过本步 DPO 更新")
                    step += 1
                    continue
                ...
                self._backward_and_step(dpo_loss, self.dpo_optimizer, step_scheduler=True)
